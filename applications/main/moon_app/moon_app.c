...
static bool moon_app_custom_event_callback(void* context, uint32_t event) {
    furi_assert(context);
    MoonApp* app = context;
...
bool moon_app_apply(MoonApp* app) {
    ...
    if(app->show_slideshow) {
        ...
        view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewPopup);
        return true;
...
static bool moon_app_back_event_callback(void* context) {
    furi_assert(context);
    MoonApp* app = context;

    if(!scene_manager_has_previous_scene(app->scene_manager, MoonAppSceneStart)) {
        if(moon_app_apply(app)) {
            return true;
        }
    }
...
static void
    moon_app_push_mainmenu_app_raw(MoonApp* app, FuriString* label, FuriString* exe) {
...
void moon_app_push_mainmenu_app(MoonApp* app, FuriString* exe) {
    ...
    if(furi_string_start_with(exe, "/")) {
        ...
        moon_app_push_mainmenu_app_raw(app, label, exe);
    } else {
        bool found = false;
...
void moon_app_load_mainmenu_apps(MoonApp* app) {
    ...
    if(file_stream_open(stream, MAINMENU_APPS_PATH, FSAM_READ, FSOM_OPEN_EXISTING) &&
       ...
       version <= 1) {
        while(stream_read_line(stream, line)) {
            ...
            moon_app_push_mainmenu_app(app, line);
        }
    } else {
        for(size_t i = 0; i < FLIPPER_APPS_COUNT; i++) {
            furi_string_set(label, FLIPPER_APPS[i].name);
            furi_string_set(line, FLIPPER_APPS[i].name);
            moon_app_push_mainmenu_app_raw(app, label, line);
        }
...
void moon_app_empty_mainmenu_apps(MoonApp* app) {
    CharList_it_t it;
...
MoonApp* moon_app_alloc() {
    MoonApp* app = malloc(sizeof(MoonApp));
    ...
    app->scene_manager = scene_manager_alloc(&moon_app_scene_handlers, app);

    view_dispatcher_set_event_callback_context(app->view_dispatcher, app);

    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, moon_app_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, moon_app_back_event_callback);
    ...
    view_dispatcher_add_view(
        app->view_dispatcher,
        MoonAppViewVarItemList,
        variable_item_list_get_view(app->var_item_list));

    app->submenu = submenu_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, MoonAppViewSubmenu, submenu_get_view(app->submenu));

    app->text_input = text_input_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, MoonAppViewTextInput, text_input_get_view(app->text_input));

    app->byte_input = byte_input_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, MoonAppViewByteInput, byte_input_get_view(app->byte_input));

    app->number_input = number_input_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher,
        MoonAppViewNumberInput,
        number_input_get_view(app->number_input));

    app->popup = popup_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, MoonAppViewPopup, popup_get_view(app->popup));

    app->dialog_ex = dialog_ex_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, MoonAppViewDialogEx, dialog_ex_get_view(app->dialog_ex));
        ...
        while(storage_dir_read(folder, &info, name, ASSET_PACKS_NAME_LEN)) {
            if(info.flags & FSF_DIRECTORY && name[0] != '.') {
                ...
                if(app->asset_pack_index != 0) {
                    ...
                    if(strcmp(copy, moon_settings.asset_pack) == 0)
...
void moon_app_free(MoonApp* app) {
    furi_assert(app);

    // Gui modules
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewVarItemList);
    variable_item_list_free(app->var_item_list);
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewSubmenu);
    submenu_free(app->submenu);
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewTextInput);
    text_input_free(app->text_input);
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewByteInput);
    byte_input_free(app->byte_input);
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewNumberInput);
    number_input_free(app->number_input);
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewPopup);
    popup_free(app->popup);
    view_dispatcher_remove_view(app->view_dispatcher, MoonAppViewDialogEx);
