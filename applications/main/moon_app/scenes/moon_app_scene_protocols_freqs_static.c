...
void moon_app_scene_protocols_freqs_static_var_item_list_callback(
    void* context,
    uint32_t index) {
    MoonApp* app = context;
...
static void moon_app_scene_protocols_freqs_static_frequency_changed(VariableItem* item) {
...
void moon_app_scene_protocols_freqs_static_on_enter(void* context) {
    MoonApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        moon_app_scene_protocols_freqs_static_frequency_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, moon_app_scene_protocols_freqs_static_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneProtocolsFreqsStatic));

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewVarItemList);
}

bool moon_app_scene_protocols_freqs_static_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, MoonAppSceneProtocolsFreqsStatic, event.event);
        consumed = true;
        switch(event.event) {
        ...
        case VarItemListIndexAddStaticFreq:
            scene_manager_set_scene_state(
                app->scene_manager, MoonAppSceneProtocolsFreqsAdd, false);
            scene_manager_next_scene(app->scene_manager, MoonAppSceneProtocolsFreqsAdd);
            break;
        default:
            break;
        }
...
void moon_app_scene_protocols_freqs_static_on_exit(void* context) {
    MoonApp* app = context;
