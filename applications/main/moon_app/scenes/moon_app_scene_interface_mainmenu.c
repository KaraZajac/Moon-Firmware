...
void moon_app_scene_interface_mainmenu_var_item_list_callback(void* context, uint32_t index) {
    MoonApp* app = context;
...
static void moon_app_scene_interface_mainmenu_menu_style_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, menu_style_names[index]);
    moon_settings.menu_style = index;
    app->save_settings = true;
}

static void moon_app_scene_interface_mainmenu_app_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
...
static void moon_app_scene_interface_mainmenu_move_app_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
...
void moon_app_scene_interface_mainmenu_on_enter(void* context) {
    MoonApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        "Menu Style",
        MenuStyleCount,
        moon_app_scene_interface_mainmenu_menu_style_changed,
        app);
    variable_item_set_current_value_text(item, menu_style_names[moon_settings.menu_style]);
    variable_item_set_current_value_index(item, moon_settings.menu_style);

    variable_item_list_add(var_item_list, "Reset Menu", 0, NULL, app);

    size_t count = CharList_size(app->mainmenu_app_labels);
    item = variable_item_list_add(
        var_item_list, "Item", count, moon_app_scene_interface_mainmenu_app_changed, app);
    if(count) {
    ...
    item = variable_item_list_add(
        var_item_list, "Move Item", 3, moon_app_scene_interface_mainmenu_move_app_changed, app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, moon_app_scene_interface_mainmenu_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneInterfaceMainmenu));

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewVarItemList);
}

bool moon_app_scene_interface_mainmenu_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, MoonAppSceneInterfaceMainmenu, event.event);
        consumed = true;
        switch(event.event) {
        case VarItemListIndexMenuStyle:
            scene_manager_next_scene(app->scene_manager, MoonAppSceneInterfaceMainmenuStyle);
            break;
        case VarItemListIndexResetMenu:
            scene_manager_next_scene(app->scene_manager, MoonAppSceneInterfaceMainmenuReset);
            break;
        ...
        case VarItemListIndexAddItem:
            scene_manager_next_scene(app->scene_manager, MoonAppSceneInterfaceMainmenuAdd);
            break;
        default:
            break;
        }
...
void moon_app_scene_interface_mainmenu_on_exit(void* context) {
    MoonApp* app = context;
