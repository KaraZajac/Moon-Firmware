...
void moon_app_scene_protocols_var_item_list_callback(void* context, uint32_t index) {
    MoonApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void moon_app_scene_protocols_subghz_bypass_changed(VariableItem* item) {
...
static void moon_app_scene_protocols_subghz_extend_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    view_dispatcher_send_custom_event(app->view_dispatcher, VarItemListIndexSubghzExtend);
}

static void moon_app_scene_protocols_file_naming_prefix_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    bool value = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, value ? "After" : "Before");
    moon_settings.file_naming_prefix_after = value;
    app->save_settings = true;
}

void moon_app_scene_protocols_on_enter(void* context) {
    MoonApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        2,
        moon_app_scene_protocols_subghz_bypass_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        2,
        moon_app_scene_protocols_subghz_extend_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        "File Naming Prefix",
        2,
        moon_app_scene_protocols_file_naming_prefix_changed,
        app);
    variable_item_set_current_value_index(item, moon_settings.file_naming_prefix_after);
    variable_item_set_current_value_text(
        item, moon_settings.file_naming_prefix_after ? "After" : "Before");

    variable_item_list_set_enter_callback(
        var_item_list, moon_app_scene_protocols_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneProtocols));

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewVarItemList);
}

bool moon_app_scene_protocols_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, MoonAppSceneProtocols, event.event);
        consumed = true;
        switch(event.event) {
        case VarItemListIndexSubghzFreqs:
            scene_manager_set_scene_state(app->scene_manager, MoonAppSceneProtocolsFreqs, 0);
            scene_manager_next_scene(app->scene_manager, MoonAppSceneProtocolsFreqs);
            break;
        ...
        case VarItemListIndexGpioPins:
            scene_manager_set_scene_state(app->scene_manager, MoonAppSceneProtocolsGpio, 0);
            scene_manager_next_scene(app->scene_manager, MoonAppSceneProtocolsGpio);
            break;
        default:
            break;
        }
...
void moon_app_scene_protocols_on_exit(void* context) {
    MoonApp* app = context;
