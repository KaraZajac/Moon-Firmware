...
void moon_app_scene_protocols_freqs_hopper_var_item_list_callback(
    void* context,
    uint32_t index) {
    MoonApp* app = context;
...
static void moon_app_scene_protocols_freqs_hopper_frequency_changed(VariableItem* item) {
...
void moon_app_scene_protocols_freqs_hopper_on_enter(void* context) {
    MoonApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        moon_app_scene_protocols_freqs_hopper_frequency_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, moon_app_scene_protocols_freqs_hopper_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneProtocolsFreqsHopper));

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewVarItemList);
}

bool moon_app_scene_protocols_freqs_hopper_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, MoonAppSceneProtocolsFreqsHopper, event.event);
        consumed = true;
        switch(event.event) {
        ...
        case VarItemListIndexAddHopperFreq:
            scene_manager_set_scene_state(
                app->scene_manager, MoonAppSceneProtocolsFreqsAdd, true);
...
void moon_app_scene_protocols_freqs_hopper_on_exit(void* context) {
    MoonApp* app = context;
