...
void moon_app_scene_misc_screen_var_item_list_callback(void* context, uint32_t index) {
    MoonApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void moon_app_scene_misc_screen_dark_mode_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    bool value = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, value ? "ON" : "OFF");
    moon_settings.dark_mode = value;
    app->save_settings = true;
}

static void moon_app_scene_misc_screen_hand_orient_changed(VariableItem* item) {
    bool value = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, value ? "ON" : "OFF");
    if(value) {
...
static void moon_app_scene_misc_screen_rgb_backlight_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
...
static void moon_app_scene_misc_screen_lcd_color_changed(VariableItem* item, uint8_t led) {
    MoonApp* app = variable_item_get_context(item);
...
static void moon_app_scene_misc_screen_lcd_color_0_changed(VariableItem* item) {
    moon_app_scene_misc_screen_lcd_color_changed(item, 0);
}
static void moon_app_scene_misc_screen_lcd_color_1_changed(VariableItem* item) {
    moon_app_scene_misc_screen_lcd_color_changed(item, 1);
}
static void moon_app_scene_misc_screen_lcd_color_2_changed(VariableItem* item) {
    moon_app_scene_misc_screen_lcd_color_changed(item, 2);
}
static const struct {
    uint8_t led;
    VariableItemChangeCallback cb;
} lcd_cols[] = {
    {0, moon_app_scene_misc_screen_lcd_color_0_changed},
    {1, moon_app_scene_misc_screen_lcd_color_1_changed},
    {2, moon_app_scene_misc_screen_lcd_color_2_changed},
};
...
static void moon_app_scene_misc_screen_rainbow_lcd_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
static void moon_app_scene_misc_screen_rainbow_speed_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
...
static void moon_app_scene_misc_screen_rainbow_interval_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
static void moon_app_scene_misc_screen_rainbow_saturation_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item) + 1;
    char str[4];
    snprintf(str, sizeof(str), "%d", index);
...
void moon_app_scene_misc_screen_on_enter(void* context) {
    MoonApp* app = context;
    ...
    item = variable_item_list_add(
        var_item_list, "Dark Mode", 2, moon_app_scene_misc_screen_dark_mode_changed, app);
    variable_item_set_current_value_index(item, moon_settings.dark_mode);
    variable_item_set_current_value_text(item, moon_settings.dark_mode ? "ON" : "OFF");

    item = variable_item_list_add(
        var_item_list, "Left Handed", 2, moon_app_scene_misc_screen_hand_orient_changed, app);
    ...
    item = variable_item_list_add(
        ...
        "RGB Backlight",
        2,
        moon_app_scene_misc_screen_rgb_backlight_changed,
        app);
    value_index = moon_settings.rgb_backlight;
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, value_index ? "ON" : "OFF");
    ...
    for(size_t i = 0; i < COUNT_OF(lcd_cols); i++) {
        ...
        variable_item_set_locked(item, !moon_settings.rgb_backlight, "Needs RGB\nBacklight!");
    }
    ...
    item = variable_item_list_add(
        ...
        moon_app_scene_misc_screen_rainbow_lcd_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        "Rainbow Speed",
        25,
        moon_app_scene_misc_screen_rainbow_speed_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        moon_app_scene_misc_screen_rainbow_interval_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        "Rainbow Saturation",
        255,
        moon_app_scene_misc_screen_rainbow_saturation_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, moon_app_scene_misc_screen_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneMiscScreen));

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewVarItemList);
}

bool moon_app_scene_misc_screen_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, MoonAppSceneMiscScreen, event.event);
        consumed = true;
        switch(event.event) {
        case VarItemListIndexRgbBacklight: {
            ...
            bool value = variable_item_get_current_value_index(item);
            if(value == moon_settings.rgb_backlight) value = !value; // Invoked via click
            ...
            if(change) {
                moon_settings.rgb_backlight = value;
                app->save_settings = true;
                app->save_backlight = true;
        ...
        case VarItemListIndexLcdColor2:
            scene_manager_set_scene_state(
                app->scene_manager,
                MoonAppSceneMiscScreenColor,
                event.event - VarItemListIndexLcdColor0);
            scene_manager_next_scene(app->scene_manager, MoonAppSceneMiscScreenColor);
            break;
        default:
            break;
        }
...
void moon_app_scene_misc_screen_on_exit(void* context) {
    MoonApp* app = context;
