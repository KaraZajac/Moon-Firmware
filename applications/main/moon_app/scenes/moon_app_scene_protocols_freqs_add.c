...
static void
    moon_app_scene_protocols_freqs_add_number_input_callback(void* context, int32_t number) {
    MoonApp* app = context;
    ...
    bool is_hopper =
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneProtocolsFreqsAdd);
    if(is_hopper) {
...
void moon_app_scene_protocols_freqs_add_on_enter(void* context) {
    MoonApp* app = context;
    ...
    number_input_set_result_callback(
        number_input,
        moon_app_scene_protocols_freqs_add_number_input_callback,
        app,
        0,
        100000,
        999999);

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewNumberInput);
}
...
bool moon_app_scene_protocols_freqs_add_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;
...
void moon_app_scene_protocols_freqs_add_on_exit(void* context) {
    MoonApp* app = context;
