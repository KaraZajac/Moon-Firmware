...
void moon_app_scene_misc_dolphin_var_item_list_callback(void* context, uint32_t index) {
    MoonApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void moon_app_scene_misc_dolphin_dolphin_level_changed(VariableItem* item) {
...
static void moon_app_scene_misc_dolphin_dolphin_xp_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
...
static void moon_app_scene_misc_dolphin_dolphin_angry_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
...
static void moon_app_scene_misc_dolphin_butthurt_timer_changed(VariableItem* item) {
    MoonApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, butthurt_timer_names[index]);
    moon_settings.butthurt_timer = butthurt_timer_values[index];
    app->save_settings = true;
    app->save_dolphin = true;
}

void moon_app_scene_misc_dolphin_on_enter(void* context) {
    MoonApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        moon_app_scene_misc_dolphin_dolphin_level_changed,
        app);
    ...
    // show buttons and change values in callback
    item = variable_item_list_add(
        var_item_list, "Dolphin XP", 3, moon_app_scene_misc_dolphin_dolphin_xp_changed, app);
    variable_item_set_current_value_index(
        item,
        app->dolphin_xp == 0              ? 0 :
        app->dolphin_xp == DOLPHIN_MAX_XP ? 2 :
    ...
    item = variable_item_list_add(
        ...
        "Dolphin Angry",
        BUTTHURT_MAX + 1,
        moon_app_scene_misc_dolphin_dolphin_angry_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        moon_app_scene_misc_dolphin_butthurt_timer_changed,
        app);
    value_index = value_index_uint32(
        moon_settings.butthurt_timer, butthurt_timer_values, COUNT_OF(butthurt_timer_values));
    ...
    variable_item_list_set_enter_callback(
        var_item_list, moon_app_scene_misc_dolphin_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, MoonAppSceneMiscDolphin));

    view_dispatcher_switch_to_view(app->view_dispatcher, MoonAppViewVarItemList);
}

bool moon_app_scene_misc_dolphin_on_event(void* context, SceneManagerEvent event) {
    MoonApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, MoonAppSceneMiscDolphin, event.event);
        consumed = true;
        switch(event.event) {
        case VarItemListIndexDolphinXp:
            scene_manager_next_scene(app->scene_manager, MoonAppSceneMiscDolphinXp);
            break;
        default:
            break;
        }
...
void moon_app_scene_misc_dolphin_on_exit(void* context) {
    MoonApp* app = context;
