...
int32_t moon_companion_srv(void* p) {
    ...
    /* Note on boot ordering: do NOT call any ACI / ble_event_dispatcher
     * init here. This thread runs concurrently with bt_srv, and
     * gap_init() — which also calls ble_event_dispatcher_init() —
     * hasn't finished yet. Touching the event dispatcher at this point
     * dereferences NULL and boot-loops the whole device. GATT client
     * init is deferred to moon_ble_start_scan(), which only runs once
     * the user or auto-reconnect triggers a connection attempt — by
     * which time BT is fully up. */
