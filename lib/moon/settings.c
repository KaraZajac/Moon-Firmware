...
typedef enum {
    moon_settings_type_str,
    moon_settings_type_int,
    moon_settings_type_uint,
    moon_settings_type_bool,
} moon_settings_type;

static const struct {
    moon_settings_type type;
    const char* key;
    void* val;
    union {
        size_t str_len;
...
#define setting(t, n)             .type = moon_settings_type##t, .key = #n, .val = &moon_settings.n
#define setting_str(n)            setting(_str, n), .str_len = sizeof(moon_settings.n)
#define num(t, n, min, max)       .t##_min = min, .t##_max = max, .t##_sz = sizeof(moon_settings.n)
...
void moon_settings_load(void) {
    ...
    if(flipper_format_file_open_existing(file, MOON_SETTINGS_PATH)) {
        ...
        bool ok;
        for(size_t entry_i = 0; entry_i < COUNT_OF(moon_settings_entries); entry_i++) {
#define entry moon_settings_entries[entry_i]
            switch(entry.type) {
            case moon_settings_type_str:
    ...
    rgb_backlight_load_settings(moon_settings.rgb_backlight);
}

void moon_settings_save(void) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    FlipperFormat* file = flipper_format_file_alloc(storage);

    if(flipper_format_file_open_always(file, MOON_SETTINGS_PATH)) {
        int32_t tmp_int;
        uint32_t tmp_uint;
        for(size_t entry_i = 0; entry_i < COUNT_OF(moon_settings_entries); entry_i++) {
#define entry moon_settings_entries[entry_i]
